...
h1.main {
	...
	background : linear-gradient(to right, red, pink, red, pink, red, pink, red);
	/*
	HTML, CSS에서의 단위구조
...
/*
	...
	nav#main ul { }
	id가 main 인 nav Box 내부에 있는 ul tag에만
	제한적으로 적용하라
	본문의 다른곳에 있는 ul에는 속성을 적용하지 말라
*/
nav#main ul {
	margin: 0px;
	background: linear-gradient(to right, brown, pink, brown, pink, brown, pink, brown);
}
...
<script>
	...
	document.addEventListener("DOMContentLoaded", function(ev) {
		...
		이벤트 버블링 효과에 의해 
		최종적으로 상단으로 이벤트가 전파되는 것을 역 이용하여
		id main인 nav tag에 클릭 event를 선언하였다
		nav tag를 클릭하면
		선택된 li tag의 메뉴 제목을 캐치하여
		메뉴 제목에 따라 필요한 page로 전환을 한다
		*/
...
<nav id="main">
	<ul>
		<%-- 
		일반적으로 메뉴(내비게이션)등을 만들때 해당 메뉴를 클릭하면 다른 page로 점프하는 방식을
		가장 쉽게 구현하기 위하여 a tag를 사용한다
		
		a tag를 사용할 때 a tag의 기본 속성을 제거하고 새로운 속성을 부여하는
		CSS 코드를 추가한다
		
		이러한 방식이 번거로워서 아예 a tag를 사용하지 않고 다른 종류의 tag를 사용하여
		메뉴를 구성하기도 한다
		...
		스크립트를 사용하면 또다른 이점이 있는데 메뉴와 관련된 부분을 스크립트에서 모아서
		사용할 수 있고 상대적으로 html 코드에 직접 anchor를 설정하는 것보다
		본문코드가 간편해지기도 한다
	 --%>
