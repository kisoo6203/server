...
public class TodoServiceImplV1 implements TodoService {
	...
	public List<Map<String, Object>> selectAll() {
		...
		// 완료되지않은(td_edate값이 없는) 데이터를
		// 우선 보이고
		// INSERT 된 순서로 보여라
		// 미완료된 TODO가 먼저 출력되는 효과를 보임
	...
	public Integer insert(Map<String, Object> vo) {
		...
		sql += " td_sdate,"; // seq값은 auto increment되어 쓸필요가없다
		sql += " td_stime,";
		sql += " td_doit) ";
		sql += " VALUES( ?, ?, ? ) ";
	...
	private void viewSelect(ResultSet rSet) throws SQLException {
		...
		/*
		 * ResultSet으로 DB로부터 수신한데이터에서 메타데이터를 활용하여 List<VO>에 값 세팅하기
		 */
		...
		for (int i = 0; i < columns; i++) {
			System.out.print(md.getColumnName(i + 1) + "\t"); // index번째 들어있는 칼럼을 나열
		}
		System.out.println("\n" + "-".repeat(50));

		/*
		 * rSet에는 select한 table의 결과가 모두 저장되어 있다 실제데이터와 메타데이터가 모두 포함되어 있다 rSet객체의 next()
		 * method를 한번 호출하면 결과 table의 첫번째 레코드를 읽을 수 있도록 준비해준다 이어서 next() method를 계속해서
		 * 호출하면 한줄씩 순서대로 읽을 수 있도록 준비해준다 만약 더이상 읽을 데이터가 없으면 false를 return한다 while() 반복문을
